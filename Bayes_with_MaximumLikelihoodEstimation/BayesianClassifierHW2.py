...
#**************************************************************************************#     
    ...
    def determine_case_and_params(self, param_estimates=None, print_params=False, training_size='all'):    
        '''
        ...
        if param_estimates == 'ML':
            covariances = getattr(self, 'ML_cov_estimate_'+ str(training_size).replace(".", "_"))
            #covariances = self.ML_cov_estimate
            #n= len(self.ML_mu_estimate[0]) # number of features
            _mu_ = getattr(self, 'ML_mu_estimate_'+ str(training_size).replace(".", "_"))
            n = len(_mu_)
            mu = [_mu_[i].reshape(n,1) for i in range(len(_mu_))] #reshape the given mu inputs into vectors
            #mu = [self.ML_mu_estimate[i].reshape(n,1) for i in range(len(self.ML_mu_estimate))] #reshape the given mu inputs into vectors
            ...
            if show_boundary==True:
                try:
                    if len(self.boundary_function)>1:
                        ...
                        labels=['Decision Boundary', '']
                        [ax.plot(self.df.x1.sort_values(), self.boundary_function[i](self.df.x1.sort_values()), lw=1, color='k', label=labels[i]) for i in range(len(self.boundary_function))]
                except TypeError:
